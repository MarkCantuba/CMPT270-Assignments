...
public class Ward 
{
	 ...
	 */
	protected int externalToInternalIndex(int bedLabel)
	{
	 ...
	 */
	protected int internalToExternalLabel(int arrayIndex)
	{
	 ...
	 * Modified the method so that the bed lable stored in the Patient's database matches 
	...
	public void assignPatientToBed(Patient p, int bedLabel)
	{
		if (isOccupied(bedLabel)) {
			System.out.println("Bed " + bedLabel + " is Occupied!");
		}
		else {
	...
	/**
	 * getEmptyBeds() method simply returns all the bed labels that are unoccupied and available
	 */
	...
	for (Patient x: beds) {
		if (x == null) {
			holder += "Bed number " + this.internalToExternalLabel(counter) + " is not occupied.\n";
		}
		counter++;
		}
	return holder;
	...
	/**
	 * releasePatient() removes a patient from a specific bed, and marks it as null, meaning no one is inside the room
	 * sets Patient class' bedLabel number = -1 (unassigned bed default).
	 * @parameter patientName: the name of the patient that is to be released
	 */
	public void releasePatient(int bedLabel) {
		if (!isOccupied(bedLabel)) {
			System.out.println("No one is assigned to bed " + bedLabel);
		}
		else {
			beds[externalToInternalIndex(bedLabel)].setBed(-1);
	...
	public static void main(String[] args)
	{
		...
		Patient p2 = new Patient("Sue", 654321);
		w.assignPatientToBed(p2, 202);
		if (! w.isOccupied(202))
			System.out.println("Function isOccupied incorrectly returns that bed 202 is not occupied.");
		if (w.getPatient(202) != p2)
			System.out.println("Person Sue should be in bed 205, but " + w.getPatient(202) + " is.");
		if (! w.isOccupied(205))
			System.out.println("Function isOccupied incorrectly returns that bed 205 is occupied.");
		if (p1.getBedLabel() != 205 || p2.getBedLabel() != 202) {
			System.out.println("The assign patient method should be modified so that the getBedLabel method"
					+ " will now return where the patient is assigned. Double check modification to "
					+ "assignPatientToBed method");
		}
		
		
		//testing modified assignPatientToBed and removePatient methods.
		// Displaying all beds that are unoccupied!
		if (p1.getBedLabel() != 205 || p2.getBedLabel() != 202) {
			System.out.println("The assign patient method should be modified so that the getBedLabel method"
					+ " will now return where the patient is assigned. Double check modification to "
					+ "assignPatientToBed method");
		...
		System.out.println("***************************************");
		System.out.println("Additional Tests to added to Ward.");
		System.out.println("***************************************");
		System.out.println("**Displaying beds that are unoccupied (205 and 202 are occupied)**");
		System.out.println(w.getEmptyBeds());
		
		w.releasePatient(205);
		w.releasePatient(202);
		
		if (p1.getBedLabel() != -1 && w.getPatient(205) != null) {
			System.out.println("Pete should now be released out of bed 205. Double check releasePatient() method");
		}
		if (p2.getBedLabel() != -1 && w.getPatient(202) != null) {
			System.out.println("Pete should now be released out of bed 202. Double check releasePatient() method");
		}
		
		System.out.println("***************************************");
		System.out.println("**Displaying beds that are unoccupied after releasing patients in 205 and 202**");
		System.out.println(w.getEmptyBeds());
		System.out.println("***************************************");
	}
}
